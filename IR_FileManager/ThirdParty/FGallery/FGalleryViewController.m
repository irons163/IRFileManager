...
- (void)resizeImageViewsWithRect:(CGRect)rect
{
	...
	for (i = 0; i < count; i++) {
        ...
        //LogMessage(nil, 0, @"resizeImageViewsWithRect : %d", i);
	}
}
...
- (void)setUseThumbnailView:(BOOL)useThumbnailView
{
    ...
    UIBarButtonItem *newBackButton = [[UIBarButtonItem alloc] initWithTitle: NSLocalizedString(@"Done", @"") style: UIBarButtonItemStyleDone target: self action: @selector(closeClk:)];
//    [[self navigationItem] setBackBarButtonItem: newBackButton];
    self.navigationItem.leftBarButtonItem = newBackButton;
    [newBackButton release];

    UIBarButtonItem *deleteBackButton = [[UIBarButtonItem alloc] initWithTitle: NSLocalizedString(@"Delete", @"") style: UIBarButtonItemStyleBordered target: self action: @selector(deleteClk:)];
    self.navigationItem.rightBarButtonItem = deleteBackButton;
    [deleteBackButton release];
...
- (void)buildViews
{
	...
	for (i = 0; i < count; i++) {
        ...
        //LogMessage(nil, 0, @"buildViews : %d", i);
	}
}
...
- (void)buildThumbsViewPhotos
{
	...
	for (i = 0; i < count; i++) {
        ...
        //LogMessage(nil, 0, @"buildThumbsViewPhotos : %d", i);
	}
}
...
- (void)arrangeThumbs
{
	...
	for (i = 0; i < count; i++) {
        ...
        //LogMessage(nil, 0, @"arrangeThumbs : %d", i);
	}
...
{
	// show thumb view
	[self toggleThumbnailViewWithAnimation:FALSE];
...
- (void)loadAllThumbViewPhotos
{
	...
	for (i=0; i < count; i++) {
        ...
        //LogMessage(nil, 0, @"loadAllThumbViewPhotos : %d", i);
	}
}
...
- (void)didReceiveMemoryWarning {
    ...
    if (_isThumbViewShowing==YES) {
        for (i = 0; i < count; i++)
        {
            ...
            FGalleryPhotoView *photoView = [_photoViews objectAtIndex:i];
//            [photoView.imageView.image release];
