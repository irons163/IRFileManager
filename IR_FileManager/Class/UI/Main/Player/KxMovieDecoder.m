...
- (id)init {
    if (self=[super init]) {
        ...
        _dispatchQueue = dispatch_queue_create("KxMovieDecoder", DISPATCH_QUEUE_SERIAL);
...
//        kxMovieError videoErr = [self openVideoStreamNew];
//        kxMovieError audioErr = [self openAudioStreamNew];
...
- (kxMovieError) openVideoStreamNew
{
...
- (kxMovieError) openAudioStreamNew
{
    int ret, stream_index;
    AVStream *st;
    AVCodecContext *dec_ctx = NULL;
...
int interruptCallBack(void *ctx){
    KxMovieDecoder *me = (__bridge KxMovieDecoder *)ctx;
...
//                ((self.position+1)>=(self.duration-1)))   
//            {
//                _isEOF = YES;
//                break;
//            }
...
//                
////                if(_videoCodecCtx->codec->capabilities&CODEC_CAP_TRUNCATED)
////                    _videoCodecCtx->flags|= CODEC_FLAG_TRUNCATED; /* we do not send complete frames */
//                
...
////                    pgm_save(_videoFrame->data[0], _videoFrame->linesize[0],
////                             _videoFrame->width, _videoFrame->height, buf);
//////                    (*frame_count)++;
////                }
//
//                
