...
-(UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.fileType == VIDEO_TYPE){
        ...
        cell.imageViewe.image = [EnShareTools getMusicCover:filePath];
        if(cell.imageViewe.image==nil)
            cell.imageViewe.image = [[DataManager sharedInstance] getImageOfflineWithType:@"MUSIC" ext:[cell.filenameLabel.text pathExtension]];
    }else if(self.fileType == DOCUMENT_TYPE){
        cell.imageViewe.image = [[DataManager sharedInstance] getImageOfflineWithType:@"DOCUMENT" ext:[cell.filenameLabel.text pathExtension]];
        ...
        if([type isEqualToString:@"VIDEO"]){
            ...
            @autoreleasepool {
                [cell.imageViewe setImageWithURL:[NSURL fileURLWithPath:filePath] placeholderImage:[UIImage imageNamed:@"img_photo.jpg"]  options:0 completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
                }];
            }
        }else {
            cell.imageViewe.image = [[DataManager sharedInstance] getImageOfflineWithType:type ext:[cell.filenameLabel.text pathExtension]];
        }
    }
...
-(void)setThumbToFavoritesCell:(MyFavoritesCollectionViewCell*)cell byVideoPath:(NSString*)filePath{
    ...
    cell.imageViewe.image = [[DataManager sharedInstance] getImageOfflineWithType:@"VIDEO" ext:[cell.filenameLabel.text pathExtension]];
    
    NSOperation *operation = [NSBlockOperation blockOperationWithBlock:^{
        UIImage *image = [EnShareTools generateThumbImage:filePath];
...
- (void)uploadClick{
    ...
    /* //detect if login recently then upload, else show login window.
     ...
     if (resultDictionary) {
     if ([resultDictionary[@"CheckAliveResult"] isEqualToString:@"OK"]) {
     [self getUSBInfo]; //upload to first disk partition.
     }
     if ([resultDictionary[@"CheckAliveResult"] isEqualToString:@"ERROR"]) {
...
-(void)getUSBInfo{
    ...
    [global getStorageInfo:^(NSDictionary *resultDictionary) {
        if (resultDictionary != nil) {
            ...
            if ([tempDict count]==0) {
                //                usbNameLabel.text = _(@"No mounted device");
                //                NSLocalizedString(@"USB", nil);
                [self showLoginDialog];
...
- (void)doUpload{
    ...
    for (CollectionDataFile *item in selectedItems){
        //        CollectionDataFile *item = items[indexPath.row];
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(self.fileType == VIDEO_TYPE){
        ...
        cell.thumbnailImageView.image = [EnShareTools getMusicCover:filePath];
        if(cell.thumbnailImageView.image==nil)
            cell.thumbnailImageView.image = [[DataManager sharedInstance] getImageOfflineWithType:@"MUSIC" ext:[cell.titleLabel.text pathExtension]];
    }else if(self.fileType == DOCUMENT_TYPE){
        cell.thumbnailImageView.image = [[DataManager sharedInstance] getImageOfflineWithType:@"DOCUMENT" ext:[cell.titleLabel.text pathExtension]];
    }else if(self.fileType == PHOTO_TYPE){
        ...
        if([file.type isEqualToString:@"VIDEO"]){
            ...
            cell.thumbnailImageView.image = [[DataManager sharedInstance] getImageOfflineWithType:file.type ext:[cell.titleLabel.text pathExtension]];
        }
    }
    ...
    cell.createDateLabel.text = [EnShareTools formatDate_yyyyMMdd:[EnShareTools getFileCreationTimeFromPath:filePath]];
...
-(void)setThumbToCell:(DocumentTableViewCell*)cell byVideoPath:(NSString*)filePath{
    ...
    cell.thumbnailImageView.image = [[DataManager sharedInstance] getImageOfflineWithType:@"VIDEO" ext:[cell.titleLabel.text pathExtension]];
