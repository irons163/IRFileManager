    ...
    if(cell.imageview.image == nil)
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
            ...
            if(isFirstLoad || [[self.collectionView indexPathsForVisibleItems] containsObject:indexPath]){
                dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
                [cell.imageview setImageWithURL:[NSURL fileURLWithPath:filepath] placeholderImage:[UIImage imageNamed:@"img_photo.jpg"]  options:0 completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
                    if (image)
                        return;
                    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_HIGH, 0), ^{
                        //縮小photo
...
- (void)doUpload{
    // 上傳
    BOOL tagIfUpload = NO;
    for (CollectionDataFile *item in selectedPhotos){
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
